...
export default async function ClientPage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  return <>
    ...
    <div className="two-column">
      ...
      <aside className="card detail-card"><h2>Client details</h2><dl><div><dt>Primary contact</dt><dd>{client.contact}</dd></div><div><dt>Website</dt><dd><a href={`https://${client.website}`} target="_blank" rel="noreferrer">{client.website}</a></dd></div><div><dt>Location</dt><dd>{client.location}</dd></div></dl><button className="button secondary full">Edit client</button></aside>
    </div>
  </>;
}
